...
<body class="bg-white-to-r from-blue-900 via-indigo-900 to-purple-900 py-20">
  <div class="max-w-md mx-auto px-6 text-center">
    <h2 class="text-4xl font-bold text-white mb-8">লগইন</h2>

    <!-- Login Form Container -->
    <div id="login-box" class="bg-slate-50 backdrop-blur-lg rounded-2xl p-8 border border-black transform transition-all duration-500 hover:scale-105 hover:shadow-2xl">
      ...
      <form action="{{ route('user.login') }}" method="POST" class="text-left">
        ...
        <div class="mb-4">
          ...
          <input type="tel" name="mobile_number" id="mobileNumber" value="{{ old('mobile_number') }}" class="w-full px-4 py-3 rounded-lg bg-white/20 text-black border-black border-2 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="আপনার নিবন্ধিত মোবাইল নম্বর লিখুন" required>
        </div>
        <div class="mb-4 relative">
          <label for="password" class="block text-black mb-2">পাসওয়ার্ড</label>
          <div class="relative">
            <input type="password" name="password" id="password" class="w-full px-4 py-3 rounded-lg bg-white/20 text-black border-black border-2 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="আপনার পাসওয়ার্ড লিখুন" required>
            <span class="absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer" onclick="togglePasswordVisibility()">
              <i id="eye-icon" class="fas fa-eye text-black"></i>
            </span>
          </div>
        </div>
